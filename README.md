...
项目「编写 PRD」入口现已打开独立全屏 Block 编辑器。支持模板大纲、逐块 AI 生成/修改、人工批注、自动保存、历史与版本冲突保护；提交时汇总为现有流程可读取的评审快照。使用与升级说明见 [Block PRD 编辑器](docs/block-prd-editor.md)。下方原 Gitea 评论流程仍适用于旧版 PRD；Block 文档的修改在新编辑器内完成。
