...
| Row (column) | `shape=tableRow;horizontal=0;startSize=0;swimlaneHead=0;swimlaneBody=0;fillColor=none;collapsible=0;dropTarget=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;fontSize=12;` | Empty child of table, `parent=tableId`; its label must be empty |
| Field cell | `shape=partialRectangle;html=1;whiteSpace=wrap;connectable=0;strokeColor=inherit;overflow=hidden;fillColor=none;` | The one child of a Row; it owns the plain-text field label and uses the row's absolute width and height |
| PK column | Bold text: `fontStyle=1` on the field cell | Mark with `PK` prefix or key icon |
...
**Required table hierarchy:** every ER field uses an empty `tableRow` followed by
exactly one `partialRectangle` child cell. Never put a field label on a table row:
draw.io renders that text vertically. The field cell must use the table row's
absolute `width` and `height`, without `relative="1"`: a 1×1 relative child is
effectively invisible in the viewer.

```xml
<mxCell id="customer-id-row" value="" style="shape=tableRow;horizontal=0;startSize=0;swimlaneHead=0;swimlaneBody=0;fillColor=none;collapsible=0;dropTarget=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;fontSize=12;" vertex="1" parent="customer">
  <mxGeometry y="30" width="180" height="30" as="geometry" />
</mxCell>
<mxCell id="customer-id-cell" value="PK 客户编号" style="shape=partialRectangle;html=1;whiteSpace=wrap;connectable=0;strokeColor=inherit;overflow=hidden;fillColor=none;fontStyle=1;" vertex="1" parent="customer-id-row">
