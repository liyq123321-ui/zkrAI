...
10. 底部以「浏览全文」替换「AI 修改」。进入全文审核模式前先保存草稿；全文提供只读文字视图、预览及目录定位，保留右侧队列和全部批注。全文行范围可跨模块，保存时转换成各模块的原始行号和版本；未选范围时批注全文，也可勾选目录限定模块。修改仍可通过「按此批注修改」或返回分块编辑完成。正文/背景变动时先「更新全文」，防止批注贴错位置。
11. 全文模式底部的「审核全文」发起一次完整文档审核，检查跨模块一致性、遗漏、范围和验收标准，保存摘要、问题级别、相关模块及改进建议。它不改写正文、修改块版本或结构片段，不自动批准或提交。意见在刷新后保留；审核期间或结束后正文/背景变化，会标明旧版本意见。全文请求超过 30 万字符时明确拒绝，不静默截断。失败可再次点击「审核全文」重试，运行中的审核可在队列取消。
...
- `CodexProvider` 复用现有 `CodexStructuredRunner`。块编辑仅提供当前块、当前反馈、文档背景/规则及必要的上游摘要；仅在用户主动点击「审核全文」时，专用 `review_document` 合同提供全部模块正文，返回只读整体审核报告。两种操作均不提供聊天历史。使用现有 Codex CLI 配置和登录状态。可通过 `create_app(block_provider=...)` 注入其他实现。
...
- 后端相关回归 113 项、前端相关测试 35 项通过；TypeScript 检查和生产构建通过。
- 覆盖一次传入全部模块、报告持久化、重复请求幂等、只读结果不可应用为正文、权限/版本/容量检查、取消、非法模块引用、审核期间与完成后变更、跨模块行范围映射及过期批注保护。
- 接口 `POST /prd-documents/{document_id}/review` 使用请求 ID 和期望文档版本；作业类型为 `document_review`，复用现有数据库表与队列。
