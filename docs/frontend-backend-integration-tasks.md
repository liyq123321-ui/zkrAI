...
> 状态更新（2026-09-02）：核心第一阶段已完成自动化回归。本清单保留早期分阶段计划；以下增量记录当前“跳过澄清并生成 PRD”交付，不把仍属于 Mock 蓝图的代码执行或部署能力标记为完成。

## 当前增量：跳过澄清并生成 PRD

- [x] 后端增加公开 `skip_clarification` 动作及状态门禁。
- [x] 在单事务中关闭当前澄清、推进到 `SPECIFICATION` 并保存命令回执。
- [x] 保存 `AGENT_DISCRETION` 决策和 `CLARIFICATION_SKIPPED` 审计事件。
- [x] PRD 生成提示词停止追问、使用保守假设并显式列出重要假设。
- [x] WebGUI 增加显式跳过按钮，并自动串联 `create_spec`。
- [x] WebGUI 识别少量明确跳过短语，普通澄清回答不误判。
- [x] 后端状态/集成测试与前端纯函数测试覆盖。
- [x] 用户指南、前后端工程文档和接口契约同步。
...
- [x] 增加网络离线、超时、请求取消、422、409、503 的统一 UI 状态。
