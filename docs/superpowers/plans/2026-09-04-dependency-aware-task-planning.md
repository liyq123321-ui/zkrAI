...
- [x] **Step 3: Implement deterministic topological waves**
...
- [x] **Step 4: Write failing dependent-closure and contract tests**
...
- [x] **Step 6: Implement closure and canonical contract serialization**
...
- [x] **Step 1: Write a failing dependent-start-order test**
...
- [x] **Step 2: Write a failing contract-payload test**
...
- [x] **Step 4: Schedule one topological wave at a time**
...
- [x] **Step 5: Add authoritative dependency contracts to `_plan_task`**
...
- [x] **Step 6: Bind checkpoint adoption to upstream hashes**
...
- [x] **Step 7: Update the planner prompt**
...
- [x] **Step 1: Write a failing upstream-repair cascade test**
...
- [x] **Step 2: Write a failing leaf-repair isolation test**
...
- [x] **Step 3: Write a failing recovery-hash test**
...
- [x] **Step 5: Expand upstream repair targets to dependent closure**
...
- [x] **Step 6: Keep evidence mapping deterministic**
...
- [x] **Step 1: Write a failing enrichment-order test**
...
- [x] **Step 2: Write a failing enrichment-repair cascade test**
...
- [x] **Step 4: Replace round-wide gather with shared planning waves**
...
- [x] **Step 5: Add dependency contracts to enrichment calls**
...
- [x] **Step 6: Run focused and correction tests and verify GREEN**
...
- [x] **Step 1: Run the dependency-planning regression group**
...
- [x] **Step 2: Run the complete backend suite under deterministic test identity**
...
- [x] **Step 5: Commit any test-driven corrections**
...
Completed on 2026-09-04 in the isolated
`codex/staged-decomposition` worktree. The final-review fix wave added explicit
coverage for complete execution prerequisite sets, initial enrichment dependent
closure, authoritative plan-free peer context, durable rejection recovery,
non-executable dependency targets, persisted dependency order, canonical review
evidence, and empty pre-planner evidence.

Final-review code commits:

- `8437da9` — enforce every execution prerequisite.
- `1d18e6d` — harden dependency validation, snapshot evidence, and Reviewer
  recovery.
- `b710ec1` — make enrichment contracts and evidence deterministic.

Command-level verification evidence:

- Dependency regression group, run from `backend` with
  `/Users/tangtang/Desktop/zkrAI/backend/.venv/bin/pytest -q
  tests/unit/test_task_plan_graph.py
  tests/integration/test_decomposition_service.py
  tests/integration/test_semantic_breakdown_repair.py
  tests/integration/test_agent_spec_details.py
  tests/integration/test_agent_spec_detail_corrections.py`: `145 passed`, two
  pre-existing deprecation warnings, exit `0` in 6.33 seconds.
- Complete backend suite, run from `backend` with
  `env FIRSTFLIGHT_IDENTITY_MODE=legacy
  /Users/tangtang/Desktop/zkrAI/backend/.venv/bin/pytest -q`: `598 passed`, the
  same two warnings, exit `0` in 23.60 seconds.
- Frontend tests, typecheck/lint, and production build, run from
  `frontend/aios-main` with `npm test && npm run lint && npm run build`: 7 test
  files and 59 tests passed; TypeScript emitted no diagnostics; Vite transformed
  1,946 modules and built successfully in 1.42 seconds; exit `0`.
