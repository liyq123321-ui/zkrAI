...
同步 HTTP 命令采用分级超时：普通本地事务为 130 秒；单 Agent 命令为 975 秒；可能串联生成与审核两个 Agent 的命令为 1950 秒。这个预算略大于后端每次 Agent 调用 900 秒的上限，避免后端仍在工作时浏览器提前取消。客户端会把内部计时器触发报告为可重试的 `REQUEST_TIMEOUT`，把页面卸载或调用方主动取消保留为 `REQUEST_ABORTED`。超时后工作区会立即重新读取 Session；若状态版本已经推进，则直接恢复后端结果。
