...
- 看板与流转图共享同一组四个筛选控件（搜索、Root 多选、类型、执行者），但各自维护独立状态；Kanban 仅裁剪卡片集合，Task Flow 还会移除隐藏节点、空项目泳道，以及任一端被隐藏的依赖边。任务详情、来源 PRD、审核命令、状态版本与草稿始终绑定卡片所属 Session，不依赖左侧当前对话。
...
同步 HTTP 命令采用分级超时：普通命令为 130 秒；单 Agent 命令为 2075 秒；可能串联生成与审核两个 Agent 的命令为 4150 秒。每次后端 Agent 调用上限为 2000 秒，因此这些预算覆盖正常成功路径，避免后端仍在工作时浏览器提前取消；它们不覆盖所有结构化输出重试或拆解语义修复轮次的理论最大时长。此类长路径可返回可重试的 `REQUEST_TIMEOUT`，随后沿用现有状态刷新与重试恢复。页面卸载或调用方主动取消仍保留为 `REQUEST_ABORTED`；超时后工作区会立即重新读取 Session，若状态版本已经推进则直接恢复后端结果。
...
- **图形渲染与交互实现**:
  ...
  - **筛选投影与边裁剪**:
    Task Flow 复用与 Kanban 相同的四个筛选输入，但保持独立的 `flowFilters` 状态；可见子图先移除被筛掉的任务，再隐藏没有任何可见任务的项目泳道，并跳过任一端已不可见的依赖边，确保图上只保留连续可解释的执行链路。
  - **节点交互与状态感知**:
