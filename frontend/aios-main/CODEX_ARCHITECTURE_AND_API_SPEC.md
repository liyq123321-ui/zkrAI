...
同步 HTTP 命令采用分级超时：普通命令为 130 秒；单 Agent 命令为 2075 秒；可能串联生成与审核两个 Agent 的命令为 4150 秒。每次后端 Agent 调用上限为 2000 秒，因此这些预算可避免后端仍在工作时浏览器提前取消。客户端会把内部计时器触发报告为可重试的 `REQUEST_TIMEOUT`，把页面卸载或调用方主动取消保留为 `REQUEST_ABORTED`。超时后工作区会立即重新读取 Session；若状态版本已经推进，则直接恢复后端结果。
