...
export function WorkItemDialog({ title, children, onClose, contentKey }: {
  title: string; children: ReactNode; onClose: () => void; contentKey?: string;
}) {
  ...
  return <dialog ref={ref} aria-label={title} onCancel={(event) => { event.preventDefault(); onClose(); }}
    ...
    <header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-slate-800 bg-slate-950 px-5 py-4">
      ...
      <button ref={closeButton} type="button" autoFocus aria-label="关闭详情" onClick={onClose} className="rounded-lg p-2 text-slate-300 hover:bg-slate-800"><X className="h-5 w-5" /></button>
    </header>
